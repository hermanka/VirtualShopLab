...
class Users
{
   ...
   function create_user($username, $pass, $firstname, $lastname, $vuln = False)
   {
	 ...
	 $query = sprintf("INSERT INTO `users` (`id`, `login`, `password`, `firstname`, `lastname`, `salt`, `tradebux`, `created_on`, `last_login_on`) VALUES (NULL, '%s', SHA1('%s'), '%s', '%s', '%s','%d', NOW(), NOW());",
      ...
      {         
         $usercoup = (strlen($username)>4)?substr($username,0,4):$username;
        $new_user_id = mysql_insert_id();
         $querycoup = sprintf("INSERT into `coupons` (`code`, `discount`,`user_id`) VALUES ('%s', '50', '%s');",
                        str_shuffle(strtoupper($salt.$usercoup)),
                       $new_user_id);
         if ($res = mysql_query($querycoup))
         {                   
